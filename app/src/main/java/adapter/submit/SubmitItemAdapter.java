 ...
 * @updateDes ${TODO}
 */
public class SubmitItemAdapter extends BaseSubmitFragmentAdapter {
    private List<ExercisesBean> mChoiceList;
    private final DBUtil mDbUtil;
    private ViewHolder mHolder;
    private final boolean mIsExplain;

    public SubmitItemAdapter(Context context, List<ExercisesBean> list) {
        super(context);
        mChoiceList = list;
    ...
    public void initView(ViewHolder holder, int position) {
        ...
        if (choice.selectedAnswer == null
                || TextUtils.isEmpty(choice.selectedAnswer.trim()) || db == null || db.userAnswer == null
                || TextUtils.isEmpty(db.userAnswer.trim())
                ) {
